...
std::ostream &operator<<(std::ostream &_os,
  ...
  switch (fdt_parser::get_fmt(_iter.prop_name)) {
    ...
    case fdt_parser::FMT_UNKNOWN: {
      printf("%s: (unknown format, len=0x%X)", _iter.prop_name, _iter.prop_len);
      break;
    }
    // 空
    ...
    case fdt_parser::FMT_U32: {
      printf("%s: 0x%X", _iter.prop_name, ntohl(*(uint32_t *)_iter.prop_addr));
      break;
    }
    // 64 位整数
    case fdt_parser::FMT_U64: {
      printf("%s: %lu", _iter.prop_name, ntohll(*(uint64_t *)_iter.prop_addr));
      break;
    }
    // 字符串
